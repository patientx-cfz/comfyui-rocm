...
* Updated rocm and pytorch URL's to the new "rocm10" nightly repo ; we are using this because stable has some problems atm. All supported GPU's should have up-to-date packages available.
* Added a hybrid MiniMax H3 workflow. You can get it from "sample-workflows" folder. It is using my fork of "[H3 SLA Attention](https://github.com/patientx/comfyui-h3-sla-attention-rocm)"of this [ethanfel's fork](https://github.com/ethanfel/ComfyUI-PlagueKind-Nodes-only-sparse) which itself is a fork of part of PlagueKind's awesome [nodepack](https://github.com/PlagueKind/Comfyui-PlagueKind-Nodes). When used with H3 it is much faster than sage-attention WITH RDNA2 at least, you can of course try the wf with newer gpu's and find it is useful for those. Make sure to add at least ck-attention for RDNA3+ before the SLA node, that doesn't work with RDNA2 and below but is the best choice atm for newer gpus. Willing users can help here by running this command (copy-paste,press enter) inside the comfyui-rocm folder in the commandline and sending the .json file to me in the pinned issue. `python_env\python.exe custom_nodes\comfyui-h3-sla-attention-rocm\sla_autotune_probe.py --full` (the resulting .json file would in the same folder so : "custom_nodes\comfyui-h3-sla-attention-rocm\sla_probe_gfx1***.json")
...
(make sure to select "Desktop development with C++" as shown in the screenshot below.)
<img width="1233" height="620" alt="image" src="https://github.com/user-attachments/assets/8bd483a3-9ffa-4329-8605-3d74cafd3c90" />
